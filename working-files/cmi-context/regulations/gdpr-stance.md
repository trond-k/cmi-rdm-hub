...
<!-- The institution's interpretive positions on GDPR as it applies to research data.
     Covers lawful basis, consent, special categories, data minimisation, etc.
     This is a DRAFT — positions should be reviewed and refined with input from
     CMI's legal and research ethics advisers. -->
...
- Contact details for the researcher and CMI's data protection contact
...
1. **Anticipate at the project level.** Based on the research topic, methods, and participant groups, assess which special categories are likely to arise. A project on political participation will involve political opinions. A project on refugee health will involve health data and ethnic origin. Flag these in the Data Inventory.
...
**Classification tier mapping**: special category data is typically classified as Red (Confidential) or Black (Strictly Confidential) under CMI's data classification scheme, depending on the context and the risk of harm if exposed. An interview about health in Norway: Red. An interview about health in a context where the condition is criminalised or stigmatised: Black. See the Data Security Policy for the full classification criteria and corresponding storage requirements.
...
3. **Proportional retention.** Don't retain identifiable data longer than needed. Once analysis is complete and the data has been anonymised or the retention period has passed, identifiable data should be deleted.
...
Personal data should be retained for the duration of the research project plus a reasonable verification period (typically 5 years after project completion, consistent with Research Council of Norway expectations and good scientific practice). After this period, data should either be:

- **Anonymised** and retained for potential future use or archiving, or
- **Deleted** if anonymisation is not possible or not warranted.

For data archived in a repository for reuse, the retention period is governed by the repository's policies and the consent given for archiving (if consent was the basis for the archiving decision).

Personal identifiers (names, contact details, linking keys) should be deleted as soon as they are no longer needed for the research — ideally once transcription, quality control, and any follow-up contact are complete. De-identified and anonymised research data should be retained for at least **10 years** after project completion, consistent with Research Council of Norway guidelines and standard scientific practice, and preferably longer through deposit in internal and/or external repositories and archives. These defaults align with the main funder expectations — NFR (10 years), ERC/Horizon Europe (grant duration plus verification period, typically covered by 10 years), and journal replication requirements (typically 10 years). Where a specific funder imposes a different period, the funder requirement takes precedence and should be documented in the DMP.

### Deletion in practice

Deletion means deletion — not just removing a file from a folder. It includes:
- Removing data from active storage, backups, and any cloud services
- Deleting or anonymising the anonymisation key (if pseudonymised data is being retained without the key)
- Confirming with any collaborators or partners who hold copies that they have also deleted

Researchers should plan for deletion from the start — documenting what will be deleted, when, and by whom in the DMP.
...
## A note on Sikt notification

Sikt notification is required for research projects that process personal data. Since most CMI research involves identifiable participants, most CMI projects will need a Sikt notification.

The notification is **not an approval** — Sikt does not approve or reject research designs. It is a notification that data processing will take place, accompanied by a description of the processing and the safeguards in place. Sikt's role is to assess whether the described processing is consistent with data protection requirements and to advise the institution if it identifies concerns.

Researchers should file the Sikt notification early in the project — ideally before data collection begins. The RDM Hub's Sikt Notification Form Guide can help with this.

CMI has an institutional agreement with Sikt (formerly NSD) for privacy services in research (*personverntjenester for forskning*). Under this agreement, CMI has a designated institutional contact person and access to Sikt's institutional portal (*Institusjonsportalen*), which provides an overview of all notified projects and their status. CMI also has an internal guide for researchers on how to fill out the Sikt notification form. Institutional defaults that apply to all CMI notifications include: public interest (Article 6(1)(e)) as the lawful basis, CMI as the data controller, and the RDM Hub's Sikt Notification Form Guide as the starting point for preparing the submission. See the Institutional Context document (`context.md`) for the full description of CMI's Sikt arrangement.
