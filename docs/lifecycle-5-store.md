...
- **GDPR data residency.** Microsoft stores CMI's MS365 data in EU/EEA data centres, which satisfies GDPR residency requirements for most purposes. The question becomes relevant when you move data outside the default infrastructure: sharing files through a non-EU cloud service, transferring data to a partner institution in a country without an EU adequacy decision (i.e. one whose data protection standards the EU has not recognised as essentially equivalent to the GDPR), or using a specialist platform hosted outside Europe. If your project involves any of these, check the legal basis for the transfer before data moves. See [GDPR and legal compliance](gdpr-and-legal-compliance.md).
